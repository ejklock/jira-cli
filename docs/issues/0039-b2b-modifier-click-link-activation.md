...
Delivered with `compose_detail()` as the single geometry source (one pass
yields rendered lines + per-row link-cell metadata; one wrap implementation)
and wires the `theme::link()` color onto href spans (B2a follow-up closed).

**Known follow-up (review observation):** comment-body links render the
`[url]` token but carry no hit metadata — a modifier-click on them resolves
to `None` (they were never activatable pre-B2; extend activation to comment
bodies if wanted).
